...
class InferencePipeline:
    def __init__(
        self, model, dataset, feature_dim, batch_size=4, num_workers=4, device="cuda"
    ):
        self.model = model
...
def calculate_top_k(
    q_matrix: np.ndarray, db_matrix: np.ndarray, top_k: int = 10
) -> np.ndarray:
...
def main():
    ...
    assert (
        query_path == "" and datasets_path == ""
    ), "Please specify the path to the query and datasets"
    ...
    database_pipeline = InferencePipeline(
        model=model, dataset=database_dataset, feature_dim=4096
    )
    query_pipeline = InferencePipeline(
        model=model, dataset=query_dataset, feature_dim=4096
    )
    ...
    top_k_matches = calculate_top_k(
        q_matrix=query_global_descriptors, db_matrix=db_global_descriptors, top_k=10
    )

    # record query_database_matches
    record_matches(
        top_k_matches, query_dataset, database_dataset, out_file="./LOGS/record.txt"
    )

    # visualize top-k matches
    visualize(
        top_k_matches, query_dataset, database_dataset, visual_dir="./LOGS/visualize"
    )
